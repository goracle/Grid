...
void CartesianCommunicator::ShmInitGeneric(void){
  assert(GlobalSharedMemory::ShmAlloc()==0);
#if 1
  int mmap_flag =0;
#ifdef MAP_ANONYMOUS
...
#endif
  std::cout << GridLogMessage << "Allocating mpi comm buff (non-mpi3)" << std::endl;
  ShmCommBuf =(void *) mmap(NULL, MAX_MPI_SHM_BYTES, PROT_READ | PROT_WRITE, mmap_flag, -1, 0); 
  if (ShmCommBuf == (void *)MAP_FAILED) {
    perror("mmap failed ");
    exit(EXIT_FAILURE);  
  }else{
  std::cout << GridLogMessage << "Allocation successful: mpi comm buff (non-mpi3)" << std::endl;
  }
#ifdef MADV_HUGEPAGE
  ...
  std::cout << GridLogMessage << ": mpi comm buff (non-mpi3)" << std::endl;
  GlobalSharedMemory::WorldShmCommBufs.resize(1);
  GlobalSharedMemory::WorldShmCommBufs[0] = ShmCommBuf;
  assert(GlobalSharedMemory::ShmAllocBytes(MAX_MPI_SHM_BYTES)==MAX_MPI_SHM_BYTES);
  assert(GlobalSharedMemory::ShmAlloc(1)==1);
}
